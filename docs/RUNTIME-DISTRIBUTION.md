...
Prvá verzia podporuje jeden `@startuml` blok bez preprocessor/include direktív. Sekvenčné a class diagramy sú overené skutočným rendererom. C4 makrá/includes, iné PlantUML bloky, plná kompatibilita layoutov a lokálny Mermaid zatiaľ nie sú podporované. Pri nepodporovanom zdroji je chyba a dostupný zdrojový kód, bez automatického odoslania.
